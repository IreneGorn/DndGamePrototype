...
public class DiceSidesEditor : Editor
{
    ...
    private void CalculateSides()
    {
        ...
        List<DiceSide> foundSids = FindDiceSides(mesh);
        sides.Sides = new DiceSide[foundSids.Count];
        serializedObject.Update();

        for (int i = 0; i < foundSids.Count; i++)
        {
            DiceSide side = foundSids[i];
    ...
    private Mesh GetMesh(DiceSides sides)
    {
